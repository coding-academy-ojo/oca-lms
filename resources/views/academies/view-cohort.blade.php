...
@section('content')
        ...
        <div class="row mt-4">
            <div class="col-lg-6 mb-4">
                <div class="card shadow-sm">
                    <div class="card-body">
                      <div class="d-flex w-100 align-items-start justify-content-between flex-wrap gap-3">
    ...
    <div class="ms-auto d-flex flex-column gap-2">
        ...
        {{-- Export Overall Attendance PDF (same style, placed right under Cohort Absence) --}}
        <form action="{{ route('attendance.overall.export.pdf') }}" method="GET" class="m-0">
            <input type="hidden" name="cohort_id" value="{{ session('cohort_ID') }}">
            <input type="hidden" name="introduction" value="">
            <button type="submit" class="btn btn-primary">
                Export Overall Attendance PDF
            </button>
        </form>
                            ...
                            @if($cardsStatistics['cohort'])
                                ...
                                <ul class="list-group list-group-flush">
                                    <li class="list-group-item">Start Date:
                                        <strong>{{ $cardsStatistics['cohort']->cohort_start_date }}</strong></li>
                                    <li class="list-group-item">End Date:
                                        <strong>{{ $cardsStatistics['cohort']->cohort_end_date }}</strong></li>
                                    <li class="list-group-item">Donor:
                                        <strong>{{ $cardsStatistics['cohort']->cohort_donor }}</strong></li>
                                </ul>
                            @else
                                ...
                                <p class="fs-5 text-dark">
                                    ...
                                    @if (in_array(Auth::guard('staff')->user()->role, ['trainer', 'manager']))
                                        ...
                                        @php
                                            $encryptedCohortId = Crypt::encryptString($cohort->id);
                                        @endphp
                                        <a href="{{ route('absence', ['cohort_id' => $encryptedCohortId]) }}"
                                            class="text-primary mt-2">cohort absence</a>
                            ...
                            <div class="d-flex gap-2 flex-column">
                                @if (in_array(Auth::guard('staff')->user()->role, ['trainer', 'manager']))
                                    @php
                                        $encryptedCohortId = Crypt::encryptString($cohort->id);
                                    @endphp
                                    <a href="{{ route('technology.satisfaction', ['cohort_id' => $encryptedCohortId]) }}"
                                        class="btn btn-primary">Satisfaction</a>
                                @endif
                            </div>
                        </div>
                                                                ...
                                                                <tr>
                                                                    ...
                                                                    <td>
                                                                        <div style="width: 100%; background-color: #ddd;">
                                                                            <div
                                                                                style="height: 20px; width: {{ $technology['percentage'] }}%; background-color: {{ $technology['status'] == 'Finished' ? '#4CAF50' : ($technology['status'] == 'In Progress' ? '#ffa500' : '#f44336') }};">
                                                                            </div>
                                                                        </div>
                                                                        {{ $technology['status'] }}
                                                                    </td>
                                                                    {{-- <td class="justify-content-center">
                                                                        <button type="button" class="btn btn-primary btn-sm" data-bs-toggle="modal"
                                                                            data-bs-target="#feedbackModal{{ $index + 1 }}"><span
                                                                                class="material-symbols-outlined">
                                                                                open_in_new
                                                                            </span></button>
                                                                    </td> --}}
                                                                    {{-- <div class="satisfaction-container"
                                                                        style="width: 100px; background-color: #f0f0f0;border-radius: 5px;">
                                                                        <div class="satisfaction-bar" style="width:100%; background-color: #4CAF50;"
                                                                            style="height: 100%; transition: width 0.5s ease-in-out;"></div>
                                                                    </div> --}}
                                                                    <td>
                                                                        <div class="progress"
                                                                            style="height: 20px; background-color: #f0f0f0; border-radius: 5px;">
                                                                            <div class="progress-bar" role="progressbar" style="width: {{ $technology['satisfactionRate'] }}%; 
                                                                                          background-color: 
                                        ...
                                        @foreach($softSkillsForCohort as $index => $softSkill)
                                                                <tr>
                                                                    ...
                                                                    <td>
                                                                        <div class="progress"
                                                                            style="height: 20px; background-color: #f0f0f0; border-radius: 5px;">
                                                                            <div class="progress-bar" role="progressbar" style="width: {{ $softSkill->satisfaction }}%; 
                                                                                          background-color: 
            ...
            <div class="modal-dialog modal-dialog-centered">
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        ...
                        <p class="mt-3">Note: React has been well-received, with a majority of students finding it very
                            beneficial.</p>
                    </div>
                </div>
            </div>
        ...
        <div class="row">
            <div class="col-12">
                <div id="cohort-milestone" class="card shadow-sm">
                    <h2 id="cohort-milestone-heading" style="text-align:start" class="my-4 mx-3 h3 text-primary">Cohort
                        Milestone</h2>
