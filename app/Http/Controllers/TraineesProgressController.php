...
use App\Imports\MasterpieceDetailsImport;
use App\Imports\MasterpieceProgressImport;
...
class TraineesProgressController extends Controller
{
    ...
    private function lateAssignmentSubmissions()
    {
        ...
        // it breacks if the latestAssignment is null this should be handled 
    ...
    // ========== EXPORTS (templates, cohort-prefilled) ==========

    public function exportMasterpieceDetailsTemplate()
    {
        $cohortId = session('cohort_ID');
        abort_if(!$cohortId, 404, 'Cohort not selected');

        $rows = Student::where('cohort_id', $cohortId)
            ->select('id', 'en_first_name', 'en_last_name')
            ->orderBy('id')
            ->get()
            ->map(function ($s) {
                return [
        ...
        return Excel::download($export, 'masterpiece_details_template.xlsx');
    }

    public function exportMasterpieceProgressTemplate()
    {
        $cohortId = session('cohort_ID');
        abort_if(!$cohortId, 404, 'Cohort not selected');
        ...
        $rows = Student::where('cohort_id', $cohortId)
            ...
            ->map(function ($s) {
                return [
                    'student_id' => $s->id,
                    'student_name' => trim(($s->en_first_name ?? '') . ' ' . ($s->en_last_name ?? '')),
                    // You will fill valid masterpiece_task_id from tasks table
                    'masterpiece_task_id' => null,
                    // progress is optional; default to 0 if omitted on import
                    'progress' => null,
                    'notes' => null,
                ];
            })->toArray();
        ...
        $export = new class ($rows) implements FromArray, WithHeadings, WithTitle {
            ...
            public function title(): string
            {
                return 'masterpiece_progress';
            }
            public function headings(): array
            {
                return ['student_id', 'student_name', 'masterpiece_task_id', 'progress', 'notes'];
            }
            public function array(): array
            {
                return $this->rows;
        ...
        return Excel::download($export, 'masterpiece_progress_template.xlsx');
    }

    // ========== IMPORTS (cohort-locked, staff_id from auth) ==========

    public function importMasterpieceDetails(Request $request)
    {
        $request->validate([
        ...
        abort_if(!$staffId, 403, 'Unauthorized');

        $import = new MasterpieceDetailsImport($cohortId, $staffId);
        ...
        return back()->with('success', 'Masterpiece details imported successfully.');
    }

    public function importMasterpieceProgress(Request $request)
    {
        $request->validate([
            'file' => 'required|file|mimes:xlsx,csv,xls',
        ]);
        ...
        $import = new MasterpieceProgressImport($cohortId, $staffId);
        Excel::import($import, $request->file('file'));
        ...
        return back()->with('success', 'Masterpiece progress imported successfully.');
    }
